...
just open the store, search "python" and install (i used version 3.8)
once installed all you need is to open the CMD and write:
python "PATH"

python is a inline interpreter coding language so all code executed can be viewed and edited with a simple text viewer like the windows' notepad.

there are 2 files:
bot D version: all info is collected via prompts and is deleted once the program is terminated.
bot - edit: all info should be added to the code via text editor and is so it persists, i recommend visual studio code with python plugin.
